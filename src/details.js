...
// cauta parametru type si pune in const type
...
function getTort(itemID) {
  http.get("http://localhost:3000/torturi/" + itemID).then((tort) => {
    let output = "";
    output += `<div class="detailsCard">
                <img class="card-img-top" id="img" src="${tort.img}" />
...
function getPrajitura(itemID) {
  http.get("http://localhost:3000/prajituri/" + itemID).then((prajitura) => {
    let output = "";
    output += `<div class="detailsCard">
                    <img class="card-img-top" id="img" src="${prajitura.img}" />
                    <div class="contentDetails">
                        <h2 class="card-title" id="productTitle"> ${prajitura.denumire}</h2>
                        <p class="card-text">${prajitura.descriere}</p>
                        <p class="subtil">Vă rugăm să comandați cu minim 2 zile lucrătoare în avans. Ne rezervăm dreptul de a refuza comanda, dacă nu o putem onora – în acest caz, veți fi contactat telefonic în cel mai scurt timp.</p>
                        <p><span id="pretPrajitura">${prajitura.prices} Ron/kg</span></p>
                        <button type="button" id="comandaBtn" class="btn btn-primary cmdBtn">Comanda</button>
                    </div>
                </div>`;
    document.getElementById("output_details").innerHTML += output;
...
export function showSuccesMessage() {
  ...
  addMessage.innerHTML = "Produsul a fost adaugat in cosul tau!";
...
export function showErrorMessage() {
  let addMessage = document.createElement("p");
  addMessage.classList.add("addBanner");
  addMessage.innerHTML = "Selecteaza o optiune!";
...
function addToCart() {
  ...
  if (pret) {
    const img = document.getElementById("img").getAttribute("src");
